...
// Define the structure for our documents
...
export async function saveVectorStore(path: string, docs: VectorDocument[]): Promise<void> {
...
export async function loadVectorStore(path: string): Promise<VectorDocument[]> {
  try {
    const data = await fs.promises.readFile(path, 'utf-8');
    return JSON.parse(data) as VectorDocument[];
  } catch (error) {
